...
module.exports = function testFind (users, softUsers) {
	...
	test('find by id soft deletes', function (t) {
		...
		t.equal(q._boundTo.builder.toString(), 'select * from "users" where "id" = 1 and "removed_at" is null limit 1');
		t.end();
	});

	test('find by criteria', function (t) {
		var q = users.find({ email: 'test' });
		t.equal(q._boundTo.builder.toString(), 'select * from "users" where "email" = \'test\' limit 1');
		t.end();
	});
};
